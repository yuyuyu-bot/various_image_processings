...
inline void adaptive_bilateral_filter(
    ...
    const auto [kernel_space, kernel_color_table] = pre_compute_kernels<512 * 3>(ksize, sigma_space, sigma_color);
    ...
    const auto get_kernel_color = [kernel_color_table](const cv::Vec3b& a, const cv::Vec3b& b, cv::Vec3i& offset) {
    ...
    // IntegralImage src_integral(src, radius);
    ...
    struct AdaptiveBilateralFilterCore : cv::ParallelLoopBody {
        ...
        void operator()(const cv::Range& range) const CV_OVERRIDE {
            for (int r = range.start; r < range.end; r++) {
                for (int x = 0; x < width_; x++) {
                    ...
                    const auto src_sum = src_integral_.get(x - radius_, y - radius_, x + radius_, y + radius_);
                    ...
                    for (int ky = -radius_; ky <= radius_; ky++) {
                        for (int kx = -radius_; kx <= radius_; kx++) {
                            ...
                            const auto kernel    = get_kernel_space_(kx, ky) * get_kernel_color_(src_center_pix, src_pix, offset);
        ...
        const IntegralImage src_integral_;
